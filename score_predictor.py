...
class ScorePredictor:
    ...
    def _get_upcoming_games(self, sport_key: str) -> List[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                return response.json()[:10]  # Limit to 10 games
            return []
    ...
    def _calculate_prediction_confidence(self, odds_analysis: Dict, home_goals: int, away_goals: int) -> float:
        ...
        try:
            ...
            prob_difference = abs(home_prob - away_prob)
            if prob_difference > 0.3:
                base_confidence += 0.2
            elif prob_difference > 0.2:
                base_confidence += 0.1
            
            # Adjust based on bookmaker consensus
            if odds_analysis['bookmaker_count'] >= 5:
                base_confidence += 0.1
            
            # Reduce confidence for high-scoring predictions
            total_goals = home_goals + away_goals
            if total_goals > 4:
                base_confidence -= 0.1
            elif total_goals < 1:
